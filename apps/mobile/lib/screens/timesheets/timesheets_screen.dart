...
class _TimesheetsScreenState extends State<TimesheetsScreen> {
  ...
  Future<void> _load() async {
    try {
      mine = asList(await ApiClient.get('/timesheets/mine'));
    } catch (_) {}
    try {
      pending = asList(await ApiClient.get('/timesheets/pending-approval'));
    } catch (_) {}
    try {
          ...
          children: [
              ...
              SectionCard(
                ...
                child: Column(children: [
                  for (final t in pending)
                    RowTile(
                      title:
                          '${t['employee']['user']['firstName']} ${t['employee']['user']['lastName']} — ${t['type']}',
                      subtitle:
                          '${_d.format(_dt(t['periodStart']))} – ${_d.format(_dt(t['periodEnd']))}'
            ...
            SectionCard(
              ...
              trailing: FilledButton(
                  onPressed: _createDialog, child: const Text('+ New')),
              child: Column(children: [
                for (final t in mine.take(10))
                  RowTile(
                    title:
                        '${t['type']}${t['project'] != null ? ' · ${t['project']['name']}' : ''}',
                    subtitle:
                        '${_d.format(_dt(t['periodStart']))} – ${_d.format(_dt(t['periodEnd']))}'
                        '${t['actionNote'] != null ? ' · ${t['actionNote']}' : ''}',
                    trailing:
                        t['status'] == 'DRAFT' || t['status'] == 'DISCARDED'
                            ? TextButton(
                                onPressed: () => _act(
                                    () => ApiClient.post(
                                        '/timesheets/${t['id']}/submit'),
                                    'Submitted'),
                                child: const Text('Submit'))
                            : StatusBadge(t['status'].toString()),
                  ),
                if (mine.isEmpty)
                  const EmptyState('No timesheets yet — tap + New.'),
              ]),
            ),
          ]),
    );
        ...
        builder: (ctx, setD) => AlertDialog(
          ...
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            DropdownButtonFormField(
              ...
              items: const [
                DropdownMenuItem(
                    value: 'CLIENT', child: Text('Client (billable)')),
              ...
              onPressed: () {
                ...
                _act(() async {
                  final ts = await ApiClient.post('/timesheets', {
                    'type': type,
                    if (type == 'CLIENT' && projectId != null)
                      'projectId': projectId,
                    'periodStart': day.toUtc().toIso8601String(),
                    'periodEnd': day.toUtc().toIso8601String(),
                    'entries': [
                      {
                        'workDate': day.toUtc().toIso8601String(),
                        'hours': num.tryParse(hours.text) ?? 8,
                        'task': task.text
                      },
                    ],
                  });
