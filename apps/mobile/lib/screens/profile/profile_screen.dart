...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _load() async {
    try {
      me = await ApiClient.get('/employees/me') as Map<String, dynamic>?;
    } catch (_) {}
    try {
      salary = asList(await ApiClient.get('/employees/me/salary'));
    } catch (_) {}
    try {
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    Wrap(spacing: 6, children: [
                      for (final r in AuthService.roles)
                        Chip(
                            label:
              ...
              SectionCard(
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      Text(str(me!['department']?['name']),
                          style: TextStyle(color: Theme.of(context).hintColor)),
                      if (me!['manager'] != null)
                        Text('Manager: ${personName(me!['manager']?['user'])}',
                            style: TextStyle(
                                color: Theme.of(context).hintColor,
                                fontSize: 12)),
                      if (me!['joinDate'] != null)
                        Text(
                            'Joined ${_d.format(DateTime.parse(me!['joinDate'].toString()))}',
                            style: TextStyle(
                                color: Theme.of(context).hintColor,
                                fontSize: 12)),
                      const SizedBox(height: 8),
                      Wrap(spacing: 6, runSpacing: 6, children: [
                        for (final s in (me!['skills'] as List? ?? []))
                          Chip(
                              label: Text(s['skill']?['name']?.toString() ?? '',
                    ...
                    children: [
                      for (final a in me!['allocations'] as List)
                        RowTile(
                          ...
                          subtitle:
                              '${a['percentage'] ?? a['allocationPct']}% · ${str(a['project']?['location'])}',
                        ),
                    ],
                  ),
                ),
                    ...
                    children: [
                      for (final a in me!['assetAssignments'] as List)
                        RowTile(
                          ...
                          subtitle:
                              '${str(a['asset']?['category'])} · ${str(a['asset']?['model'])}',
                        ),
                    ],
                  ),
                ),
                  ...
                  children: [
                    for (final s in salary)
                      RowTile(
                        ...
                        subtitle:
                            'from ${_d.format(DateTime.parse(s['effectiveFrom'].toString()))}',
                        trailing: StatusBadge(
                            s['isOffered'] == true ? 'OFFERED' : 'REVISION'),
                      ),
                  ],
                ),
              ),
